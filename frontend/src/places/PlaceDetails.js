...
function PlaceDetails() {
	...
	async function deleteComment(deletedComment) {
		await fetch(`http://localhost:8000/places/${place.placeId}/comments/${deletedComment.commentId}`, {
			method: 'DELETE'
		})
	...
	async function createComment(commentAttributes) {
		const response = await fetch(`http://localhost:8000/places/${place.placeId}/comments`, {
			method: 'POST',
			headers: {
